	...
	int max_len)
{
	...
	pr_info("[debug][%s] %s done with opt = 0x%08x, cal_id = 0x%08x, "
	...
	void *data)
{
    ...
    pr_info("[debug][%s] %s: listen ok! ret = %d \r\n", 
		LOG_FLAG, __func__, ret);
	...
	if(TASK_STATE_RUN == sock_task_state) {
		pr_err("[debug][%s] %s: accept ok, Connection "
	...
	while(TASK_STATE_RUN == sock_task_state) {
		...
		if(0 > ret) {
			...
			pr_info("[debug][%s] %s: process = %d \r\n", 
				LOG_FLAG, __func__, ret);
		}
	}

end :
	pr_info("[debug][%s] %s: sia81xx_sock_server_task close \r\n", 
		LOG_FLAG, __func__);
	
	if(NULL != sock_clt) {
		pr_err("[debug][%s] %s: sock_release sock_clt \r\n", 
			LOG_FLAG, __func__);
...
static int sia81xx_close_sock_task(void)
{
	...
	pr_info("[debug][%s] %s: use wait time %d ms \r\n", 
...
int sia81xx_open_sock_server()
{
	...
	pr_info("[debug][%s] %s: server:socket_create ok! ret = %d \r\n", 
		LOG_FLAG, __func__, ret);
	...
	pr_info("[debug][%s] %s: server:bind ok! ret = %d \r\n", 
		LOG_FLAG, __func__, ret);
	...
	pr_info("[debug][%s] %s: socket server start ok !!! \r\n", 
		LOG_FLAG, __func__);
...
int sia81xx_close_sock_server(void)
{
	...
	pr_info("[debug][%s] %s: run !! \r\n", LOG_FLAG, __func__);

	sia81xx_close_sock_task();

	if((NULL != sock_srv) && (0 != sock_start_flag)) {
		pr_err("[debug][%s] %s: sock_release sock_srv !! \r\n", 
			LOG_FLAG, __func__);
		sock_release(sock_srv);
	}
