	...
	unsigned int owi_mode)
{
	...
	pr_info("[debug][%s] %s: running mode = %u \r\n", 
	...
	switch(simple_strtoul(strsep(&cur, split_symb), &after, 10)) {
		...
		case SIA81XX_CMD_GET_REG :
		{
			...
			if(0 != sia81xx_regmap_read(sia81xx->regmap, addr, 1, &val)) {
				pr_err("[debug][%s] %s: err regmap_read \r\n", 
					LOG_FLAG, __func__);
			} else {
				pr_info("[debug][%s] %s: addr = 0x%02x, val = 0x%02x \r\n", 
		...
		case SIA81XX_CMD_VDD_SET_OPEN :
		{
			...
			pr_info("[debug][%s] %s: set auto vdd state %u, port 0x%04x \r\n", 
					LOG_FLAG, __func__, 
					sia81xx->en_dyn_ud_vdd, 
					sia81xx->dyn_ud_vdd_port);
			break;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("[debug][%s] %s: ucontrol = %ld, rst = %d \r\n", 
	...
	sia81xx_dev_t *sia81xx)
{
	...
	pr_info("[debug][%s] %s: add sia81xx dev : %s, count = %u \r\n", 
	...
	sia81xx_dev_t *sia81xx)
{
	...
	pr_info("[debug][%s] %s: del sia81xx dev : %s, count = %u \r\n", 
		LOG_FLAG, __func__, sia81xx->name, sia81xx_list_count());
	...
	struct device_node	*sia81xx_of_node)
{
	...
	pr_info("[debug][%s] %s: get dev name : %s \r\n", 
...
static void put_sia81xx_dev(sia81xx_dev_t *sia81xx)
{
	...
	pr_info("[debug][%s] %s: put dev name : %s, pdev = %p, client = %p \r\n", 
				LOG_FLAG, __func__, 
...
static int sia81xx_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(&pdev->dev, &dev_attr_sia81xx_cmd);
	if(0 != ret) {
		pr_err("[ err][%s] %s: device_create_file fail:%d \r\n", 
			LOG_FLAG, __func__, ret);
	}
...
{	
	...
	pr_info("[debug][%s] %s: running, version : %s \r\n", 
