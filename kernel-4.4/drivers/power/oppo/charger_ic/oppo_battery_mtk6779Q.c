...
extern bool oplus_is_fg_iic_error(bool is_iic_error);
extern bool oplus_get_fg_i2c_err_occured(void);
extern void oplus_set_fg_i2c_err_occured(bool i2c_err);
extern void oplus_is_chargering(bool is_chargering);
				...
				struct device *dev)
{
	...
	if (pinfo)
		pinfo->is_fg_error_check = of_property_read_bool(np, "qcom,fg_error_check");
	...
	if (g_oppo_chip && g_oppo_chip->dual_charger_support) {
		switch (psp) {
			...
			case POWER_SUPPLY_PROP_SUB_CURRENT:
				if (g_oppo_chip->charger_exist && bq25910_is_detected()) {
					icharging = oppo_gauge_get_batt_current();
					val->intval = icharging + oppo_chg_get_main_ibat();
				} else {
					val->intval = 0;
				}
				break;
...
//====================================================================//
#endif /* VENDOR_EDIT */
static void oplus_check_fg_iic_work(struct work_struct *work)
{
	struct oppo_chg_chip *chip = g_oppo_chip;
	static int i2c_sda_error_count = 0;

	if(!chip)
		return;

	if (chip->batt_volt == 0 && chip->icharging == 0 && 
		chip->temperature == -2731) {
		i2c_sda_error_count++;
		pr_err("%s, gauge i2c sda error count=%d\n", __func__, i2c_sda_error_count);
	}

	if (i2c_sda_error_count >= 5) {
		pr_err("%s, reset gauge i2c\n", __func__);
		oplus_is_fg_iic_error(true);
		i2c_sda_error_count = 0;
	}
	if(chip->charger_exist){
		oplus_is_chargering(true);
	}else
	{
		oplus_is_chargering(false);
	}

	if(oplus_get_fg_i2c_err_occured())
	{
	...
	pr_err("%s, reset_gauge_i2c, %d\n", __func__, i2c_sda_error_count);
	schedule_delayed_work(&pinfo->check_fg_iic_work, msecs_to_jiffies(5000));
}
...
#ifdef VENDOR_EDIT
/* wangdengwen@BSP.CHG.Basic, 2020/09/23, Add for gauge test */
	if (pinfo->is_fg_error_check) {
		INIT_DELAYED_WORK(&pinfo->check_fg_iic_work, oplus_check_fg_iic_work);
		schedule_delayed_work(&pinfo->check_fg_iic_work, msecs_to_jiffies(5000));
	}
#endif
