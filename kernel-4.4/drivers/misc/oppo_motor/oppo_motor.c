...
static void position_detect_work(struct work_struct *work)
{
	...
	while (chip->motor_started) {
		...
		} else if (hall1_val > chip->cali_data.up_retard_hall1 + retard_range_end_offset) {
						...
						const char *buff, size_t count)
{
	...
	if (sscanf(buff, "%d,%d,%d,%d,%d,%d", &data[0],&data[1],&data[2],&data[3],&data[4],&data[5]) == 6) {
		...
		if (g_chip->retard_range_end) {
			g_chip->cali_data.irq_pos_hall0 = data[0]*g_chip->sign - (g_chip->retard_range_end - 5);
			g_chip->cali_data.irq_pos_hall1 = data[1]*g_chip->sign - (g_chip->retard_range_end - 5);
		} else {
...
static ssize_t dhall_retard_range_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!g_chip) {
		MOTOR_ERR("g_chip null\n");
	...
	return sprintf(buf,"%d,%d retard_range_end=%d\n", g_chip->retard_range, g_chip->retard_sign, g_chip->retard_range_end);
}

static ssize_t dhall_retard_range_store(struct device *pdev, struct device_attribute *attr,
						const char *buff, size_t count)
{
	int neg_value[2] = {0};
...
static DEVICE_ATTR(retard_range,   S_IRUGO | S_IWUSR, dhall_retard_range_show, dhall_retard_range_store);
