...
#define DEVICE_VERSION_S5K3P8SX    "s5k3p8sx"
extern void register_imgsensor_deviceinfo(char *name, char *version, u8 module_id);
static kal_uint8 deviceInfo_register_value = 0x00;
static kal_uint32 recoder_shutter;
static kal_uint8 bLongShutterFlag = 0;
#endif
...
static void write_shutter(kal_uint32 shutter)
{
	...
	/*jie.pang@camera.driver,20180108, add for slow shutter mirror*/
	kal_uint8 itemp_mirror;
	if(bNeedSetNormalMode == KAL_TRUE && bLongShutterFlag ==1) {
		bLongShutterFlag ++;
		return;
	}
	#endif
	...
	if (shutter>27340) {
		...
		write_cmos_sensor(0x0202, recoder_shutter);
		...
		/*jie.pang@camera.driver,20180108, add for slow shutter mirror mode:IMAGE_HV_MIRROR 0x03 */
		itemp_mirror = read_cmos_sensor(0x0101);
		itemp_mirror &= ~0x03;
		write_cmos_sensor(0x0101, itemp_mirror | 0x03);
		...
		if (bNeedSetNormalMode) {
			...
			write_cmos_sensor_8(0x305b, 0x00);// linelength shife
			write_cmos_sensor_8(0x304C, 0x00); // disable 2nd frame function
			bNeedSetNormalMode = KAL_FALSE;
			bLongShutterFlag = 0;
		}
		write_cmos_sensor(0x0340, imgsensor.frame_length);
		write_cmos_sensor(0x0202, shutter);
		recoder_shutter = shutter;
	}
	#endif
	LOG_INF("recoder_shutter =%d, shutter =%d, framelength =%d, bLongShutterFlag:%d\n",
		recoder_shutter, shutter,imgsensor.frame_length, bLongShutterFlag );
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			LOG_INF("s5k3p8sx read_0x0000=0x%x, 0x0001=0x%x,0x0000_0001=0x%x\n",
				read_cmos_sensor_8(0x0000),read_cmos_sensor_8(0x0001),read_cmos_sensor(0x0000));
			if (*sensor_id == imgsensor_info.sensor_id) {
				#ifdef VENDOR_EDIT
				module_id = read_module_id();
				if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_b", DEVICE_VERSION_S5K3P8SX, module_id);
			...
			switch (*feature_data) {
				...
				case MSDK_SCENARIO_ID_SLIM_VIDEO:
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0;
					break;
				case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0; // @raviprakash.dixit@oppo.com thirdparty rearcam video crash issues, 05/21/19 
					break;
				default:
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0;
					break;
