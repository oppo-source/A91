...
bool S5K4H7_update_awb(unsigned char page)
{
         ...
         golden_rg = read_cmos_sensor_8(0x0A1A)|read_cmos_sensor_8(0x0A1B)<<8;
         golden_bg = read_cmos_sensor_8(0x0A20)|read_cmos_sensor_8(0x0A21)<<8;
         ...
         LOG_INF("updata wb golden_rg=0x%x golden_bg=0x%x unit_rg=0x%x unit_bg =0x%x\n",golden_rg,golden_bg,unit_rg,unit_bg);
...
unsigned char s5k4h7_get_module_id(void)
{
	...
	lens_id = read_cmos_sensor_8(0x0A0C);
	LOG_INF("S5K4H7 module id = 0x%x , lens_id = 0x%x\n", module_id, lens_id);
	return module_id;
}
