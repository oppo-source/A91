...
#ifdef VENDOR_EDIT
/*Feng.Hu@Camera.Driver 20171213 add for evb and t0 board both work well*/
struct MCLK_PINCTRL_NAMES mclk_pinctrl_list_evb[IMGSENSOR_SENSOR_IDX_MAX_NUM][MCLK_STATE_MAX_NUM] = {
	{{"cam0_mclk_off"}, {"cam0_mclk_on"} },
	{{"cam1_mclk_off"}, {"cam1_mclk_on"} },
	{{"cam2_mclk_off"}, {"cam2_mclk_on"} },
	{{"cam3_mclk_off"}, {"cam3_mclk_on"} },
...
/* changes for 17311_P BringUP */
struct MCLK_PINCTRL_NAMES mclk_pinctrl_list_17331[IMGSENSOR_SENSOR_IDX_MAX_NUM][MCLK_STATE_MAX_NUM] = {
	{{"cam1_mclk_off"}, {"cam1_mclk_on"} },
	{{"cam2_mclk_off"}, {"cam2_mclk_on"} },
	{{"cam0_mclk_off"}, {"cam0_mclk_on"} },
	{{"cam3_mclk_off"}, {"cam3_mclk_on"} },
...
#endif
#ifdef VENDOR_EDIT
struct MCLK_PINCTRL_NAMES mclk_pinctrl_list_17061[IMGSENSOR_SENSOR_IDX_MAX_NUM][MCLK_STATE_MAX_NUM] = {
	{{"cam1_mclk_off"}, {"cam1_mclk_on"} },
	{{"cam2_mclk_off"}, {"cam2_mclk_on"} },
	{{"cam0_mclk_off"}, {"cam0_mclk_on"} },
	{{"cam3_mclk_off"}, {"cam3_mclk_on"} },
	...
	struct IMGSENSOR_HW_DEVICE_COMMON *pcommon)
{
	...
	#ifdef VENDOR_EDIT
	/*Vikrant.gupta@Camera.Driver 20100402 add for 17061 board work well*/
	PK_PR_ERR("15:13 ophf mclk_init enter\n");
	if (is_project(OPPO_17175) || is_project(OPPO_17061)) {
		PK_PR_ERR("ophf 17061/17175 board\n");
	for (i = IMGSENSOR_SENSOR_IDX_MIN_NUM; i < IMGSENSOR_SENSOR_IDX_MAX_NUM; i++) {
		if (mclk_pinctrl_list_17061[i][MCLK_STATE_DISABLE].ppinctrl_names)
			pinst->ppinctrl_state[i][MCLK_STATE_DISABLE] =
				pinctrl_lookup_state(pinst->ppinctrl,
				mclk_pinctrl_list_17061[i][MCLK_STATE_DISABLE].ppinctrl_names);
		...
		if (pinst->ppinctrl_state[i][MCLK_STATE_DISABLE] != NULL &&
			...
			PK_PR_ERR("%s : 1 pinctrl err, %s\n",
				__func__, mclk_pinctrl_list_17061[i][MCLK_STATE_ENABLE].ppinctrl_names);

			ret = IMGSENSOR_RETURN_ERROR;
		}

		if (mclk_pinctrl_list_17061[i][MCLK_STATE_ENABLE].ppinctrl_names)
			pinst->ppinctrl_state[i][MCLK_STATE_ENABLE] =
				pinctrl_lookup_state(pinst->ppinctrl,
				mclk_pinctrl_list_17061[i][MCLK_STATE_ENABLE].ppinctrl_names);
	...
	} else if (is_project(OPPO_17331))
	{
		...
		{
			PK_PR_ERR("ophf 17331 evb board\n");
			for (i = IMGSENSOR_SENSOR_IDX_MIN_NUM; i < IMGSENSOR_SENSOR_IDX_MAX_NUM; i++)
			{
				if (mclk_pinctrl_list_evb[i][MCLK_STATE_DISABLE].ppinctrl_names)
					pinst->ppinctrl_state[i][MCLK_STATE_DISABLE] =
					pinctrl_lookup_state(pinst->ppinctrl,
					mclk_pinctrl_list_evb[i][MCLK_STATE_DISABLE].ppinctrl_names);
				...
				{
					PK_PR_ERR("%s : 1 pinctrl err, %s\n",
					__func__, mclk_pinctrl_list_evb[i][MCLK_STATE_ENABLE].ppinctrl_names);
					ret = IMGSENSOR_RETURN_ERROR;
				}

				if (mclk_pinctrl_list_evb[i][MCLK_STATE_ENABLE].ppinctrl_names)
					pinst->ppinctrl_state[i][MCLK_STATE_ENABLE] =
						pinctrl_lookup_state(pinst->ppinctrl,
						mclk_pinctrl_list_evb[i][MCLK_STATE_ENABLE].ppinctrl_names);
		...
		{
			PK_PR_ERR("ophf 17331 board\n");
			for (i = IMGSENSOR_SENSOR_IDX_MIN_NUM; i < IMGSENSOR_SENSOR_IDX_MAX_NUM; i++)
			{
				if (mclk_pinctrl_list_17331[i][MCLK_STATE_DISABLE].ppinctrl_names)
					pinst->ppinctrl_state[i][MCLK_STATE_DISABLE] =
					pinctrl_lookup_state(pinst->ppinctrl,
					mclk_pinctrl_list_17331[i][MCLK_STATE_DISABLE].ppinctrl_names);
				...
				{
					PK_PR_ERR("%s : 1 pinctrl err, %s\n",
					__func__, mclk_pinctrl_list_17331[i][MCLK_STATE_ENABLE].ppinctrl_names);
					ret = IMGSENSOR_RETURN_ERROR;
				}

				if (mclk_pinctrl_list_17331[i][MCLK_STATE_ENABLE].ppinctrl_names)
					pinst->ppinctrl_state[i][MCLK_STATE_ENABLE] =
						pinctrl_lookup_state(pinst->ppinctrl,
						mclk_pinctrl_list_17331[i][MCLK_STATE_ENABLE].ppinctrl_names);
				if (pinst->ppinctrl_state[i][MCLK_STATE_ENABLE] != NULL ||
				IS_ERR(pinst->ppinctrl_state[i][MCLK_STATE_ENABLE]))
				{
					PK_PR_ERR("%s : 2 pinctrl err, %s\n", __func__,
	...
	} else if (is_project(OPPO_17197) || is_project(OPPO_18011) ||is_project(OPPO_18311) ) {
