	...
	enum led_brightness bright)
{
	...
	case MT6370_PMU_LED4:
		...
		break;
	default:
		dev_notice(led_cdev->dev, "invalid mt led index\n");
		return;
	}
	ret = mt6370_pmu_led_update_bits(led_cdev, reg_addr, reg_mask,
					 (bright & reg_mask) << reg_shift);
	if (ret < 0) {
		dev_notice(led_cdev->dev, "update brightness fail\n");
		return;
	}
	if (!bright)
		need_enable_timer = false;
	...
	if (ret < 0)
		dev_notice(led_cdev->dev, "update enable bit fail\n");
}
	...
	unsigned long *delay_on, unsigned long *delay_off)
{
	...
	if (i < 0) {
		dev_notice(led_cdev->dev, "no match, sum %lu\n", ton + toff);
		i = 0;
	}
	...
	unsigned long *delay_on, unsigned long *delay_off)
{
	...
	if (mode_sel == MT6370_PMU_LED_PWMMODE) {
		...
		if (ret < 0)
			dev_notice(led_cdev->dev, "%s: cfg pwm fail\n",
						  __func__);
	}
	ret = mt6370_pmu_led_change_mode2(led_cdev, mode_sel);
	if (ret < 0)
		dev_notice(led_cdev->dev, "%s: change mode fail\n", __func__);
	return 0;
}
	...
	unsigned long *delay_on, unsigned long *delay_off)
{
	...
	if (mode_sel == MT6370_PMU_LED_PWMMODE) {
		...
		ret = mt6370_pmu_led_change_mode(led_cdev,
						 MT6370_PMU_LED_BREATHMODE);
		if (ret < 0)
			dev_notice(led_cdev->dev, "%s: mode fix fail\n",
						  __func__);
...
static void mt6370_led_enable_dwork_func(struct work_struct *work)
{
	...
	ret =  mt6370_pmu_reg_update_bits(rgbled_data->chip,
					  ...
					  reg_mask, reg_data);
	if (ret < 0)
		dev_notice(rgbled_data->dev, "timer update enable bit fail\n");
}
...
static int mt6370_pmu_rgbled_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6370_pmu_led_trigger) && !new; i++) {
		...
		if (ret < 0) {
			dev_notice(&pdev->dev, "register %d trigger fail\n", i);
	...
	for (i = 0; i < ARRAY_SIZE(mt6370_led_classdev) && !new; i++) {
		...
		if (ret < 0) {
			dev_notice(&pdev->dev, "register led %d fail\n", i);
