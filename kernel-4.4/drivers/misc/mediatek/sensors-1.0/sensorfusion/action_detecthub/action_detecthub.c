...
#ifdef VENDOR_EDIT
//tangjh@PSW.bsp.sensor 2019/7/6 add for report event 
static struct wakeup_source oppo_actiondetect_wake_lock;
#endif
...
static int action_detect_recv_data(struct data_unit_t *event, void *reserved)
{
    ...
    if (event->flush_action == DATA_ACTION)
    {
        #ifdef VENDOR_EDIT
        //tangjh@PSW.bsp.sensor 2019/7/6 add for report event
        __pm_wakeup_event(&oppo_actiondetect_wake_lock, msecs_to_jiffies(100));
        #endif
...
static int action_detecthub_local_init(void)
{
    ...
    #ifdef VENDOR_EDIT
    //tangjh@PSW.bsp.sensor 2019/7/6 add for report event
    wakeup_source_init(&oppo_actiondetect_wake_lock, "oppo_actiondetect_wake_lock");
    #endif
    return 0;
exit:
    return -1;
