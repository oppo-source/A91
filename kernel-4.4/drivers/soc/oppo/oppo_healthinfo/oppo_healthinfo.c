...
/******  version show  ******/
static ssize_t version_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
        char page[1024] = {0};
        int len = 0;

        len = sprintf( page, "ohm_version: %s\n", cur_ohm_version);
...
static int __init oppo_healthinfo_init(void)
{
	...
	if(!oppo_healthinfo) {
        ...
        pentry = proc_create("ohm_version", S_IRUGO, oppo_healthinfo, &proc_version_fops);
        if(!pentry) {
                ohm_err("create ohm_version proc failed.\n");
                goto ERROR_INIT_VERSION;
        }
